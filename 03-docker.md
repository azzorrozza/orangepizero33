...
* Docker CE instalado pelo repositório oficial.
* Docker Compose Plugin disponível através do comando `docker compose`.
* Docker Buildx habilitado.
* Containerd utilizado como runtime.
* Estrutura de diretórios organizada em `/opt/docker`.
* Rede Docker compartilhada (`services`) criada para comunicação entre containers.
* Portainer CE instalado para gerenciamento gráfico dos containers.
* Serviço Docker iniciado automaticamente durante o boot.

---

# Observações

O Portainer armazena todas as configurações em:

```text
/opt/docker/portainer/data
```

O usuário administrador é criado apenas na primeira inicialização.

Caso seja necessário reiniciar completamente o Portainer (apagando usuários e configurações):
...
rm -rf /opt/docker/portainer/data/*
