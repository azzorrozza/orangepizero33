...
- A OrangePi conecta-se automaticamente à Tailnet durante a inicialização.
- O encaminhamento IPv4 e IPv6 permanece habilitado.
- A rede local é detectada automaticamente pela interface `end0`.
- Alterações futuras na subnet da LAN são refletidas automaticamente no anúncio das rotas.
- A OrangePi atua como **Exit Node**.
- A OrangePi atua como **Subnet Router**.
- O recurso **UDP GRO Forwarding** permanece habilitado após cada reinicialização.
...
- ✔ Sistema atualizado
- ✔ Tailscale instalado
- ✔ Dispositivo autenticado na Tailnet
- ✔ IP Forwarding habilitado
- ✔ Subnet Router configurado
- ✔ Detecção automática da subnet da LAN
- ✔ Exit Node configurado
- ✔ UDP GRO Forwarding persistente
