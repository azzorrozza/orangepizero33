...
services:
  pihole:
    ...
    # Bootstrap do Gravity (remover após a primeira inicialização)
    dns:
      - 1.1.1.1
      - 1.0.0.1
...
Aguarde o Pi-hole concluir a criação do `gravity.db`.

```bash
docker compose down
```

Edite novamente o `docker-compose.yml` e **remova** as linhas:
...
Agora configure o `systemd-resolved`:
